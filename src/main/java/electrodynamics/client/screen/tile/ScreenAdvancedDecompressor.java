...
public class ScreenAdvancedDecompressor extends GenericMaterialScreen<ContainerAdvancedDecompressor> {
    public ScreenAdvancedDecompressor(ContainerAdvancedDecompressor container, Inventory inv, Component titleIn) {
        ...
        addComponent(new ScreenComponentGeneric(ScreenComponentProgress.ProgressTextures.DECOMPRESS_ARROW_OFF, 65, 40).onTooltip(
                (graphics, component, xAxis, yAxis) -> graphics.renderTooltip(getFontRenderer(), Component.literal("1.0 / " + 1.0 / container.getHostFromIntArray().pressureMultiplier.get()), xAxis, yAxis)
        ));
        addComponent(new ScreenComponentGasGauge(() -> {
            GenericTileAdvancedCompressor.TileAdvancedDecompressor boiler = container.getHostFromIntArray();
            if (boiler != null) {
        ...
        addComponent(new ScreenComponentCondensedFluid(() -> {
            GenericTileAdvancedCompressor.TileAdvancedDecompressor generic = container.getHostFromIntArray();
            if (generic == null) {
                return null;
            }
        ...
        addComponent(new ScreenComponentButton<>(8, 75, 40, 20).setLabel(Component.literal("1/2")).setOnPress(button -> {
            GenericTileAdvancedCompressor.TileAdvancedDecompressor compressor = menu.getHostFromIntArray();
            if (compressor == null) {
                return;
            }
