...
public class ScreenAdvancedCompressor extends GenericMaterialScreen<ContainerAdvancedCompressor> {
    public ScreenAdvancedCompressor(ContainerAdvancedCompressor container, Inventory inv, Component titleIn) {
        ...
        addComponent(new ScreenComponentGeneric(ScreenComponentProgress.ProgressTextures.COMPRESS_ARROW_OFF, 65, 40).onTooltip(
                (graphics, component, xAxis, yAxis) -> graphics.renderTooltip(getFontRenderer(), Component.literal("x" + container.getHostFromIntArray().pressureMultiplier.get()), xAxis, yAxis)
        ));
        addComponent(new ScreenComponentGasGauge(() -> {
            GenericTileAdvancedCompressor.TileAdvancedCompressor boiler = container.getHostFromIntArray();
            if (boiler != null) {
        ...
        addComponent(new ScreenComponentCondensedFluid(() -> {
            GenericTileAdvancedCompressor.TileAdvancedCompressor generic = container.getHostFromIntArray();
            if (generic == null) {
                return null;
            }
        ...
        addComponent(new ScreenComponentButton<>(8, 75, 40, 20).setLabel(Component.literal("x2")).setOnPress(button -> {
            GenericTileAdvancedCompressor.TileAdvancedCompressor compressor = menu.getHostFromIntArray();
            if (compressor == null) {
                return;
            }
