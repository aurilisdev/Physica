...
import electrodynamics.prefab.screen.component.button.type.ButtonScannerPattern;
...
public class ScreenSeismicScanner extends GenericScreen<ContainerSeismicScanner> {
	...
	private final ButtonScannerPattern button;
	...
	public ScreenSeismicScanner(ContainerSeismicScanner container, Inventory inv, Component titleIn) {
		...
		button = (ButtonScannerPattern) addComponent(new ButtonScannerPattern(-AbstractScreenComponentInfo.SIZE + 1, AbstractScreenComponentInfo.SIZE + 2).onTooltip((graphics, but, xAxis, yAxis) -> {
			ButtonScannerPattern button = (ButtonScannerPattern) but;
